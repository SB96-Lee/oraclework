...
/*
    <컬럼값을 통한 산술연산>
    SELECT절 컬럼명 작성부부에 산술역산 기술할 수 있음(산술연산된 결과 조회)
*/
...
FROM EMPLOYEE;
-- > 산술연산 과정 중 null 값이 존재할 경우 산술연산한 결과값도 무조건 null 값으로 나옴

--EMPLOYEE 테이블에서 사원명, 입사일, 근무일수 조회
--DATA형식도 연산가능 : 결과값 일 단위
-- * 오늘 날짜 : SYSDATE
...
/*
 <컬럼명에 별칭 지정하기>
 산술연산 시 컬럼명이 산술에 들어간 수식 그대로 컬럼명이 됨, 이때 별칭을 부여할 수 있다
 
 [표현법]
  컬럼명 별칭 (혹은) 컬럼명 AS 별칭 (혹은) 컬럼명 "별칭" (혹은) 컬럼명 AS "별칭"
...
SELECT EMP_NAME, SALARY, BONUS, SALARY*12 "연봉(원)", (SALARY*BONUS+SALARY)*12 "보너스 포함 연봉"
FROM EMPLOYEE;
-------------------------------------------------------------------------------
/*
    <리터럴>
    ...
    SELECT절에 리터럴을 넣으면 마치 테이블 상에 존재하는 데이터처럼 조회 가능
...
SELECT TO_CHAR(EMP_ID) || EMP_NAME || TO_CHAR(SALARY) AS 종합정보
FROM EMPLOYEE;
...
--EMPLOYEE 테이블에서 급어가 350만원 이상 600만원 이하인 사원들의 사번, 사원명 급여 조회
SELECT EMP_ID, EMP_NAME, SALARY
FROM EMPLOYEE
...
--EMPLOYEE 테이블에서 급어가 350만원 미만 600만원 초과인 사원들의 사번, 사원명 급여 조회
SELECT EMP_ID, EMP_NAME, SALARY
FROM EMPLOYEE
...
-- EMPLOYEE 테이블에서 입사일이 '90/01/01 ~ 99/12/31'인 사원들의 사번, 사원명, 입사일 조회
...
/*
    ...
    [표현법]
    비교대상컬럼 LIKE '특정패턴'
    -> 특정패턴 : '%', '_' 와일드카드 사용할 수 있음   
    
    >> '%' : 0글자 이상
    EX) 비교대상컬럼 LIKE '문자%'  => 비교대상컬럼 값이 '문자'로 시작하는 데이터 조회
         비교대상컬럼 LIKE '%문자'  => 비교대상컬럼 값이 '문자'로 끝나는 데이터 조회
         비교대상컬럼 LIKE '%문자%'  => 비교대상컬럼 값이 '문자'가 포함되어 있는 데이터 조회
    'oracle%' => oracle, oracle1, oracle2....
    
    >> '_' : 1글자
    ex) 비교대상컬럼 LIKE '_문자' => 비교대상컬럼 값이 '문자'앞에 무조건 한글자가 있는 데이터 조회
    ex) 비교대상컬럼 LIKE '__문자' => 비교대상컬럼 값이 '문자'앞에 무조건 두 글자가 있는 데이터 조회
    ex) 비교대상컬럼 LIKE '_문자_' => 비교대상컬럼 값이 '문자'앞에 무조건 한글자, 뒤에도 무조건 한글자가 있는 데이터 조회
 */
 ...
 --EMPLOYEE테이블에서 전화번호 3번쨰 자리가 '1'인 사원들의 사번 , 사원명, 전화번호 조회
 SELECT EMP_ID, EMP_NAME, PHONE
 FROM EMPLOYEE
 WHERE PHONE LIKE '__1%';
 
  --EMPLOYEE테이블에서 이메일에 _ 앞에 글자가 3글자인 사원의 사원명, 이메일 조회
  SELECT EMP_NAME, EMAIL
  FROM EMPLOYEE
  WHERE EMAIL LIKE '____%'; -- 언더바 4개로 인식, 글자가 4글자 이상을 가져옴, 원했던 결과가 아님.
  /*
    * '_'가 와일드카드 인지 데이터 값인지 구분지어야 함. 
...
/*
    <IS NULL / IS NOT NULL>
    컬럼값이 null일 경우, null값 비교에 사용하는 연산자
*/
...
SELECT EMP_ID, DEPT_CODE, MANAGER_ID
FROM EMPLOYEE
WHERE MANAGER_ID IS NULL;

--EMPLOYEE테이블에서 부서배치를 받지 못했지만 보너스를 받는 사원들의 사번, 사원명, 부서코드 조회
...
/*
    <IN/NOT IN>
    IN : 컬럼값이 내가 제시한 목록 중에 일치하는 값이 있는 데이터만 조회
    NOT IN : 컬럼값이 내가 제시한 목록 중에 일치하는 값을 제외한 나머지 데이터만 조회
...
/*
    ...
    - NULLS FIRST : 생략시 DESC일때의 기본 값, 정렬하고자하는 컬럼값에 NULL이 있는 경우 데이터를 맨 앞에 배치 
    - NULLS LAST : 생략 시 ASC일때의 기본 값, 정렬하고자하는 컬럼값에 NULL이 있는 경우 데이터를 맨 뒤에 배치
*/

SELECT *
FROM EMPLOYEE
--ORDER BY BONUS; 보너스의 오름차순 정렬(null 값은 맨 마지막에 나옴)
--ORDER BY BONUS ASC; 보너스의 오름차순 정렬(null 값은 맨 마지막에 나옴)
--ORDER BY BONUS NULLS FIRST; 보너스의 오름차순 정렬(null 값은 맨 앞에 나옴)

--ORDER BY BONUS DESC; 보너스의 내림차순 정렬(null값은 맨 앞에 나옴)
ORDER BY BONUS DESC, SALARY; -- 보너스의 내림차순 정렬하고, 같다면 급여를 내림차순으로 정렬하시오
