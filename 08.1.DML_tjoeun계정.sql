...
/*
    ...
    그래도 한 행 단위로 추가됨
    -> 선택이 안된 컬럼은 NULL이 들어감. DEFAULT가 지정되어 있으면 DEFAULT 값이 들어감
...
CREATE TABLE EMP_HSAL
AS SELECT EMP_ID, EMP_NAME, SALARY, BONUS
FROM EMPLOYEE
WHERE 1=0;

CREATE TABLE EMP_LSAL
AS SELECT EMP_ID, EMP_NAME, SALARY, BONUS
FROM EMPLOYEE
WHERE 1=0;
...
INTO EMP_HSAL VALUES(EMP_ID, EMP_NAME, SALARY, BONUS)
WHEN SALARY < 3000000 THEN 
INTO EMP_LSAL VALUES(EMP_ID, EMP_NAME, SALARY, BONUS)
...
/*
    2. UPDATE
     ...
     SET 컬럼명 = 바꿀값,
         ...
         ...
     [WHERE 조건];
     -> 주의 : WHERE절을 생락하면 테이블안의 모든 컬럼값이 변경됨
*/
--테이블 생성
...
--지정보 사원의 급여를 2백만원으로 인상, 보너스 10프로
UPDATE EMP_SALARY
SET SALARY = 2000000,
    BONUS = 0.1
...
--왕정보 사원의 급여와 보너스를 지정보 사원과 동일하게 인상해줌
UPDATE EMP_SALARY
