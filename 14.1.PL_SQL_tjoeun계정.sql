/*
    *PL / SQL
    ...
    SQL 문장 내에서 변수 저의, 조건처리(IF), 반복문(LOOP, FOR, WHILE)등을 지원함
    
    *PL/SQL 구조
    -[선언부 (DECLARE SECTION)] : DECLARE로 시작, 변수나 상수 초기화하는 부분
...
/* 200번 사원을 조회하여 변수에 담음
    SELECT EMP_ID, EMP_NAME, SALARY
       INTO EID, ENAME, SAL            --변수에 SELECT한 결과를 넣어준거임
    FROM EMPLOYEE
    WHERE EMP_ID = 201;
*/    
...
--사용자로부터 EMP_ID 받기
    SELECT EMP_ID, EMP_NAME, SALARY
       INTO EID, ENAME, SAL          
    FROM EMPLOYEE
    WHERE EMP_ID = &사번; --사용자한테 입력값 받기 = '&' + 사용자한테 보일 글자
...
DECLARE
    E EMPLOYEE%ROWTYPE;
BEGIN
    SELECT *   --ROWTYPE을 넣을떄는 무조건 모든것을 넣어야함
    INTO E
    FROM EMPLOYEE
    WHERE EMP_ID = &사번;
...
WHERE TABLE_NAME = 'DEPARTMENT' AND COLUMN_NAME = 'DEPT_TITLE';
...
--4)   
    CASE 비교대상자
        WHEN 비교할값1 THEN 실행내용1 --> CASE문에는 세미콜론 일일이 안붙힘
...
--사용자로부터 입력받은 사번의 사원 부서코드를 이용하여 부서이름으로 출력하기
...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE(EMP.EMP_NAME || '의 근무지 국가는 ' || NNAME);
...
/*
    ...
    [표현식]
    FOR 변수 IN 초기값..최종값  --> 반대일 경우에는 IN REVERSE 초기값..최종값
    LOOP
        반복적으로 실행할 구문
    END LOOP;
*/
...
/*
    ...
    *시스템 예외(오라클에서 미리 정의해둔 예외)
    ...
    -TOO_MANY_ROWS : SELECTㅎ나 결과가 여러 행일 경우
...
--사수번호 200번은 여러명, 201번은 1명(이것만 정상출력 됨), 204번 여러명
DECLARE
    EID EMPLOYEE.EMP_ID%TYPE;
    ENAME EMPLOYEE.EMP_NAME%TYPE;
BEGIN
...
--1.  1. 사원의 연봉을 구하는 PL/SQL 블럭 작성. 보너스가 있는 사원은 보너스도 포함(IF문으로)
DECLARE
 EID EMPLOYEE.EMP_ID%TYPE;
 ENAME EMPLOYEE.EMP_NAME%TYPE;
 SAL EMPLOYEE.SALARY%TYPE;
...
BEGIN
    ...
    IF BONUS = 0
    THEN DBMS_OUTPUT.PUT_LINE('연봉 :' || SAL*12);
    ELSIF BONUS > 0
    THEN DBMS_OUTPUT.PUT_LINE('연봉 :' || SAL*(1+BONUS)*12);
    END IF;
END;
/
