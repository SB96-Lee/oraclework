/*
    <트리거 TRIGGER>
    내가 지정한 테이블에 INSERT, UPDATE, DELETE 등의 DML문에 의해 변경사항이 생겼을 때(테이블에 이벤트가 발생 했을 때
    ...
    입출고가 발생 했을 때 재고수량을 매번 UPDATE해야할 때
...
INSERT INTO TB_PROSALE
VALUES(SEQ_SCODE.NEXTVAL, 201, SYSDATE, 10, 3000000); 

COMMIT;

--출고시 수량이 부족하면 출고 안되게 하는 트리거
/*
    *사용자 함수 예외처리
    RAISE_APPLICATION_ERROR([에러코드],[에러메세지])
     - 에러코드 : -20000 ~ -20999 사이의 숫자만 사용 가능
*/
CREATE OR REPLACE TRIGGER TRG_SALE
AFTER INSERT ON TB_PROSALE
FOR EACH ROW --행이 일어날떄마다
DECLARE
    SCOUNT NUMBER;
BEGIN 
    SELECT STOCK_QUANT
...
BEGIN 
    ...
    THEN
        ...
        WHERE PCODE = :NEW.PCODE;
    ELSIF SCOUNT >= :NEW.AMOUNT AND :NEW.STATUS = '출고'
    THEN
        UPDATE TB_PRODUCT
        SET STOCK_QUANT = STOCK_QUANT - :NEW.AMOUNT
        WHERE PCODE = :NEW.PCODE;
...
--오류 
INSERT INTO TB_PRODETAIL
VALUES(SEQ_DCODE.NEXTVAL, 200, SYSDATE, 40, '출고');