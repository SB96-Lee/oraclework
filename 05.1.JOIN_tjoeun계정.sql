/*
    ...
    조회결과는 하나의 결과물(result set = 테이블)로 나옴
    
    -관계형 데이터베이스는 최소한의 데이터로 각각 테이블에 담고 있음
     (중복을 최소화하기 위해)
     => 관게형 데이터베이스에서는 SQL문을 이용한 테이블간의 "관계"를 맺는 방법
...
SELECT EMP_ID 사번, EMP_NAME, E.JOB_CODE, JOB_NAME
...
-- 해결방법2) JOIN USING구문 사용하는 방법(두 컬럼이 일치할 떄만 사용 가능)
SELECT EMP_ID, EMP_NAME, JOB_CODE, JOB_NAME
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE);

--[참고]
--NATURAL JOIN : 공통된 컬럼을 알아서 자동으로 매칭시켜줌
--공통된 컬럼은 한번만 반환(중복일 경우)
SELECT EMP_ID, EMP_NAME, JOB_CODE, JOB_NAME
FROM EMPLOYEE
NATURAL JOIN JOB;
...
--3) FULL | OUTER | JOIN : 두 테이블이 가진 모든 행 조회(오라클 전용구문은 사용불가)
...
/*
    *비등가 조인
    매칭시킬 컬럼에 대한 조건 작성 '='(등호)를 사용하지 않는 JOIN문
    ANSI구문으로는 JOIN ON으로만 가능
*/
--모든 사원들의 사원명, 급여, 급여등급을 조회
--> 오라클 전용 구문
...
FROM EMPLOYEE E, DEPARTMENT, JOB J, LOCATION, SAL_GRADE
WHERE DEPT_CODE = DEPT_ID
  AND E.JOB_CODE = J.JOB_CODE
  AND LOCATION_ID = LOCAL_CODE
