...
SELECT EMP_NAME, FLOOR((HIRE_DATE-SYSDATE)*-1) 근무일수1, FLOOR(SYSDATE-HIRE_DATE) 근무일수2
FROM EMPLOYEE;
...
SELECT EMP_ID, 
    ...
    TO_CHAR((SUBSTR(EMP_NO,1,6) + SUBSTR(EMP_NO,8,7)), 'FM9,999,999') 합
FROM EMPLOYEE
WHERE EMP_ID = 201;

-- 10. EMPLOYEE테이블에서 부서코드가 D5인 직원의 보너스 포함 연봉 조회
SELECT EMP_NAME, DEPT_CODE, TO_CHAR(NVL(SALARY*(BONUS+1)*12, SALARY*12), 'FM999,999,999') 보너스포함연봉
FROM EMPLOYEE
WHERE DEPT_CODE = 'D5';
