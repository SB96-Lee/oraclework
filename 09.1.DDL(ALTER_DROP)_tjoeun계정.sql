...
--오류: 컬럼값에 문자가 포함되어 있음. 컬럼의 타입을 변경하려면 값들을 모두 지워야 변경 가능
ALTER TABLE DEPT_COPY MODIFY DEPT_ID NUMBER;

--오류: 컬럼의 타입을 변경하려면 값들을 모두 지워야 변경 가능
ALTER TABLE EMPLOYEE_COPY MODIFY EMP_ID NUMBER;

--오류 "cannot decrease column length because some value is too big" 컬럼값이 10BYTE 넘는게 있음
...
--오류 : 한 테이블에 적어도 최소한 1개의 컬럼은 가지고 있어야 한다
ALTER TABLE DEPT_COPY2
