...
/*
    *서브쿼리의 구분
    ...
    - 단일행 서브쿼리 : 서브쿼리를 실행한 결과가 오로지 1개일 때(한행 한열)
    - 다중행 서브쿼리 : 서브쿼리를 실행한 결과 여러행 일 떄(여러행 한열)
...
/*
    *다중행 서브쿼리
    ...
    - < ANY 서브쿼리 : 여러개의 결과값 중 "한개라도" 작은 경우
                    즉, 결과값 중 가장 큰 값 보다 작은 경우
    - ALL : 서브쿼리의 값들 중 가장 큰값보다 더 큰값을 얻어올 때
*/

-- 홍정보 또는 지정보 사원과 같은 같은 직급을 가진 사원들의 사번, 사원명, 직급코드, 급여 조회
--1. 조정연 또는 지정보 사원의 직급
SELECT JOB_CODE
FROM EMPLOYEE
...
JOIN JOB USING(JOB_CODE)
WHERE JOB_NAME = '대리'
 AND SALARY > ANY(2200000, 25000000, 37600000);
...
--차장 직급임에도 과장 직급의 가장 큰 급여보다 적게 받는 사원의 사번, 사원명, 직급명, 급여 조회
SELECT EMP_ID, EMP_NAME, JOB_NAME, SALARY
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE)
WHERE JOB_NAME = '차장'
...
SELECT EMP_ID, EMP_NAME, SALARY*NVL(1+BONUS, 1)*12 포괄연봉, DEPT_CODE
FROM EMPLOYEE
WHERE SALARY*NVL(1+BONUS, 1)*12 >= 30000000;

--FROM 절 안에 서브쿼리를 넣어서 테이블처럼 사용 가능
--본절 FROM -> 서브쿼리 FROM -> 서브쿼리 SELECT -> 본절 WHERE -> 본절 SELECT 순서
--WHERE 절에 별칭을 쓰고 싶을 경우 사용
SELECT *
FROM (SELECT EMP_ID, EMP_NAME, SALARY*NVL(1+BONUS, 1)*12 포괄연봉, DEPT_CODE
      FROM EMPLOYEE)
WHERE 포괄연봉 >= 30000000;

--FROM절의 서브쿼리 안에 있는 컬럼들만 사용가능
SELECT EMP_NAME, 포괄연봉
FROM (SELECT EMP_ID, EMP_NAME, SALARY*NVL(1+BONUS, 1)*12 포괄연봉, DEPT_CODE
      FROM EMPLOYEE)
WHERE 포괄연봉 >= 30000000;

--FROM절의 서브쿼리 안에 JOB_CODE 컬럼이 없어서 오류남
SELECT EMP_NAME, 포괄연봉, job_code
FROM (SELECT EMP_ID, EMP_NAME, SALARY*NVL(1+BONUS, 1)*12 포괄연봉, DEPT_CODE
      FROM EMPLOYEE)
WHERE 포괄연봉 >= 30000000;

----------------->> 인라인 뷰를 주로 사용하는 예) TOP_N분석(상위 몇위까지만 가져오기)
--전 직원들중 급여가 가장 많이 받는 사원의 상위 5위까지 가져오기
-----> *ROWNUM : 오라클에서 제공해주는 컬럼, 조회된 순서대로 1위부터 순번을 부여해주는 컬럼
SELECT ROWNUM, EMP_NAME, SALARY
FROM EMPLOYEE;

--우선 SELECT의 이름과 급여를 가져와서 그 결과에 번호 매긴 후 급여의 내림차순 정렬
--이렇게 할 경우 FROM -> WHERE -> SELECT -> ORDER순서라서(급여 순서대로 ROWNUM 컬럼과는 상관 없이) ROWNUM 순서가 뒤죽박죽됨.
SELECT ROWNUM, EMP_NAME, SALARY
FROM EMPLOYEE
WHERE ROWNUM < 6
...
/*
    *WITH
    ...
    -장점
     같은 서브쿼리가 여러 번 사용될 경우 중복 장성을 피할 수 있다.
     실행속도도 약간 더 빠름.
*/
...
/*
    *순위 매기는 함수
    ...
    - RANK() OVER(정렬기준) : 동일한 순위가 이후의 등수를 동일한 인원수 만큼 건너뛰어 순위 계산
                            EX) 공동 1위가 2명이면 다음 순위는 3위
    - DENSE_RANK() OVER(정렬기준) : 동일한 순위가 이후의 등수를 무조건 1 증가 시킴
...
FROM EMPLOYEE;
--공동 19위가 2명  -> 그 다음은 21위

SELECT EMP_NAME, SALARY, DENSE_RANK()OVER(ORDER BY SALARY DESC)순위
FROM EMPLOYEE;
--공동 19위가 2명  -> 그 다음은 20위
